...
export default function createElement(document, keyDetail, showMore) {
  const { network, accountNo, address, publicKey, createdAt, passphraseImage = {} } = keyDetail
    ...
    <div class="row">
      <div class="col-4 offset-4">
        <a href="#" class="btn btn-link text-success ${
          showMore ? 'd-none' : ''
        }" id="clickShowMore">Show More</a>
      </div>
    </div>
    <div class="${showMore ? '' : 'd-none'}" id="moreInfo">
      <div class="row my-2 border-bottom py-3" >
        <div class="col-sm-4 text-capitalize text-strong text-field-head">
          Account Number
        </div>
  ...
  const moreInfo = div.querySelector('#moreInfo')
